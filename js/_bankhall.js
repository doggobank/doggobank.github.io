...
async function callPageTries(){
	...
	//use: getFixture, getBetsOnFixture, getResultOnFixture
	try{
		var fixtureDetails = await tokenInst.methods.getFixtureDetail().call();
		var played = fixtureDetails[0];
		var kickoff = parseFloat(fixtureDetails[1]);
		var played = fromWeiToFixed8(fixtureDetails[0]);
